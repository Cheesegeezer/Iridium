...
namespace Iridium.APICalls
{
    ...
    class APIQueries : BaseApiClient 
    {
        ...
        public ItemsResult AcclaimedQuery(Guid guid)
        {
            string query = "&format=Json"; //must include "&format=Json" in order to allow for the items to be read.
            string queryUrl = string.Format("{0}Items?MinCriticRating=80&Recursive=true&UserId={1}{2}", APIUrl(), guid, query); //Query Format taken from Swagger
            return Kernel.ApiClient.GenericApiQuery(queryUrl);//Interrogate the API based on the query string.
        }
